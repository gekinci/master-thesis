...
The transition models of the nodes and the dependencies between them are modelled as continuous-time Bayesian network (CTBN), denoted by S. \\%The network S represents a stochastic process over a structured multivariate state space $ \rchi = [\rchi_1,..., \rchi_n] $. \\
...
The belief state, if represented as a probability distribution over states, provides a summary over the agent's past experiences, and allows the agent to account for its uncertainty for its decisions. The policy of a POMDP agent leads to optimal behaviour as a function of the agent's belief state. %Since the belief state is a probability distribution with continuous values, POMDP can be considered as continuous space belief MDP. \\
...
In this application, the particles to represent the belief state are drawn from marginalized CTBN. The algorithm for belief state update through particle filtering and marginal process is given in the following chapter. %TODO something like, the proposal distribution is marginalized distribution of CTBN, relate the following to the example.
