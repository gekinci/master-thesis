...
\end{align}
where the joint summary statistics of $ X_n $ over all $ K $ trajectories are denoted by  $ T_{n}(x_i\mid u) = \sum_{\kappa=1}^{K} T_{n}^\kappa(x_i\mid u) $ and $ r_{n}(x_i, x_j\mid u) =\sum_{\kappa=1}^{K} r_{n}^\kappa(x_i, x_j\mid u)$.\\
