...
The belief state, if represented as a probability distribution over states, provides a sufficient stratistic over the agents past experiences. 
...
%TODO write about the dependency problem, how is this okay?
...
\begin{algorithm}[H]
	...
	\KwIn{Structure of the network with N local variables $ X_{1}, X_{2}, ..., X_{n} $ as a dict in the format of $ \left\lbrace X_{i}: [par(X_{i})]\right\rbrace $ with state-space $ \rchi_n = \left\lbrace x_{1}, ..., x_{m} \right\rbrace $   \\Transition intensity matrices $ \textbf{Q}_{n} $ with entries $ q^n_{i,j}$\\$ T_{max} $ to terminate simulation}
