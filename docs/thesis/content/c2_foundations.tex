...
The expected amount of future rewards upon executing a policy is defined as \textit{value function} and used to evaluate a given policy. \\%$ V_\pi(s) $ denotes the expected future reward obtained by following a policy $ \pi $, starting from state $ s $.\\
...
\end{align}
\crefrange{eq:bu1}{eq:bu2} are obtained following Bayes' theorem, and \crefrange{eq:bu2}{eq:bu3} follow from the law of total probability. \\
...
\end{align}
where the joint summary statistics of $ X_n $ over all $ K $ trajectories are denoted by  $ \Upsilon_{n}(x_i\mid u) = \sum_{\kappa=1}^{K} \Upsilon_{n}^\kappa(x_i\mid u) $ and $ r_{n}(x_i, x_j\mid u) =\sum_{\kappa=1}^{K} r_{n}^\kappa(x_i, x_j\mid u)$.\\
