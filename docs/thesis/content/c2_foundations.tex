...
\end{align}
where the summary statistics $ \Upsilon(\cdot) $ and $ r(\cdot) $, introduced in  \crefrange{eq:sum_stat_ups}{eq:sum_stat_r}, are also conditioned on parent nodes.
...
The expected amount of future rewards upon executing a policy is defined as \textit{value function} and used to evaluate a given policy. \par%$ V_\pi(s) $ denotes the expected future reward obtained by following a policy $ \pi $, starting from state $ s $.\\
...
%If the agent has only one step to take, then even though it is not completely certain
...
\end{align}
\crefrange{eq:bu1}{eq:bu2} are obtained following Bayes' theorem, and \crefrange{eq:bu2}{eq:bu3} follow from the law of total probability. \par
...
\label{eq:b_jump}
\end{align}
where $ Z_{L} = \sum_{x_{i}\in \rchi} p(y_{L} \mid x_{i})\ b(x_{i}; t_{L}^{-}) $ is the normalization factor \cite{article}.
...
\end{align}
where the joint summary statistics of $ X_n $ over all $ K $ trajectories are denoted by  $ \Upsilon_{n}(x_i\mid u) = \sum_{\kappa=1}^{K} \Upsilon_{n}^\kappa(x_i\mid u) $ and $ r_{n}(x_i, x_j\mid u) =\sum_{\kappa=1}^{K} r_{n}^\kappa(x_i, x_j\mid u)$.\par
...
\begin{algorithm}[H]
	...
	\KwIn{Structure of the network with N local variables $ X_{1}, X_{2}, ..., X_{n} $ with state-space $ \rchi_n = \left\lbrace x_{1}, ..., x_{m} \right\rbrace $   \\Transition intensity matrices $ Q_{n} $ with entries $ q^n_{i,j}$\\$ T_{max} $ to terminate simulation}
