...
%In order to test the robustness of the inference, we added Gaussian noise drawn from $ \mathcal{N}(0, \sigma) $ to the categorical variables of the observation model $ \psi $. By doing so, we model a noisy channel behaviour, where the observation might differ from the 
%one which would result by the true observation model. 
%
