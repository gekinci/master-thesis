...
\paragraph*{Gillespie Algorithm}
\paragraph*{Thinning Algorithm}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\WHILE{$ t < T_{max} $}
		\STATE{$ \tau \sim \exp(\sum_{\forall i} \sum_{\forall j \neq k} \ q^{X_{i}}_{j,k}) $}
		\STATE{\textit{transitioning node is randomly drawn with probability}  $ P(X_{i}) = \frac{q^{X_{i}}_{i}}{\sum_{\forall i} q^{X_{i}}_{i}}$}
		\STATE{\textit{next state is randomly drawn with probability} $ P(x_{j}) = \frac{q^{X_{i}}_{i}}{\sum_{\forall i} q^{X_{i}}_{i}} $}
