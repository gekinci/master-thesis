...
\begin{algorithm}[H]
	...
	\KwOut{New set of particles $ \textbf{p}^{k} $, representing $ b(t_{k}) $}
...
This section presents the sampling algorithms used for data generation, given the intensity matrices.
...
\begin{algorithm}[H]
	...
	\KwIn{Structure of the network with N local variables $ X_{1}, X_{2}, ..., X_{n} $ as a dict in the format of $ \left\lbrace X_{i}: [par(X_{i})]\right\rbrace $ with state-space $ \rchi_n = \left\lbrace x_{1}, ..., x_{m} \right\rbrace $ with entries $ q^n_{i,j} $ \\Transition intensity matrices $ \textbf{Q}_{n} $ \\$ T_{max} $ to terminate simulation}
	\KwOut{Sample trajectory of the network}
	\Init{Initialize node values $ X_{n}(0) = x_{i} \in \rchi_n$}
	
	\begin{algorithmic}[1]
		\WHILE{$ t < T_{max} $}
		\STATE{$ \tau_{parent} \sim \exp(\sum_{\forall n} \sum_{\forall i \neq j} \ q^{n}_{i,j}) $}
		\STATE{\textit{transitioning node is randomly drawn with probability}  $ P(X_{n}) = \frac{q^{n}_{i}}{\sum_{\forall n} q^{n}_{i}}$}
		\STATE{\textit{next state is randomly drawn with probability} $ P(x_{j}) = \frac{q^{n}_{i,j}}{q^{n}_{i}} $}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\WHILE{$ i < N $}
		\STATE{\textit{the upper bound for intensity,} $ \lambda^{*}$}
		\STATE{\textit{transition time $ \tau $ drawn by }  $ u \sim U(0, 1)$ \textit{ and } $ \tau = \frac{-ln(u)}{\lambda^{*}} $}
		\STATE{$ t \leftarrow t + \tau $}
		\STATE{\textit{draw } $s \sim U(0, 1)$}
		\IF{ $ s \leq \frac{\lambda(t)}{\lambda^{*}}$}
		\STATE{\textit{sample accepted and } $ t_i = t$, $ i=i+1 $}
		\ENDIF
		\ENDWHILE
	\end{algorithmic}
	\caption{Generative CTBN}
