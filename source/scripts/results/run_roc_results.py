...
def run_auroc(n_classes, c=0, n_run=10):
    path_to_thesis = '/home/gizem/master_thesis/docs/thesis/figures'
    plt.figure()
    df_runs = pd.DataFrame(columns=['Number of trajectories', 'AUROC'])
    for method in [EXACT, PART_FILT]:
        ...
        for r in range(n_run):
            L_run = []
            df_r = pd.DataFrame(columns=['Number of trajectories', 'AUROC'])
            auroc_run = []
    ...
    df_runs.to_csv(path_to_data + f'/df_auroc_{c}.csv')
...
def run_auroc_backup(n_classes, c=0, n_run=10):
    path_to_thesis = '/home/gizem/master_thesis/docs/thesis/figures'
    plt.figure()
    df_runs = pd.DataFrame(columns=['Number of trajectories', 'AUROC'])
    for method in [EXACT, PART_FILT]:
        ...
        for r in range(n_run):
            ...
            for n in divisors(int(n_sample_per_class / n_run)):
                df_scores = pd.DataFrame()
                y_labels = None
                ...
                auroc_run += [roc_auc[c]]
            df_r['Number of trajectories'] = divisors(int(n_sample_per_class / n_run))
            df_r['AUROC'] = auroc_run
            df_r['state estimator'] = 'particle filter' if method == PART_FILT else 'exact update'
            df_runs = df_runs.append(df_r)
    ax = sns.lineplot(x="Number of trajectories", y="AUROC", hue="state estimator", style="state estimator",
                      markers=True, dashes=False, data=df_runs)
    plt.savefig(path_to_thesis + f'/AUROC_{n_sample_per_class * n_classes}samples_class{c}.pdf')
    # title_font = 22
    # label_font = 20
    # legend_font = 20
    # tick_font = 18
...
def run_aupr(n_classes, c=0, n_run=10):
    path_to_thesis = '/home/gizem/master_thesis/docs/thesis/figures'
    plt.figure()
    df_runs = pd.DataFrame(columns=['Number of trajectories', 'AUPR'])
    for method in [EXACT, PART_FILT]:
        n_sample_per_class = 500
        path_to_data = '/home/gizem/DATA/ROC_' + str(n_classes) + 'MODEL_' + method + f'_{n_sample_per_class}samples/'
        ...
        for r in range(n_run):
            L_run = []
            df_r = pd.DataFrame(columns=['Number of trajectories', 'AUPR'])
            auroc_run = []
            ...
            df_r['state estimator'] = 'particle filter' if method == PART_FILT else 'exact update'
            df_runs = df_runs.append(df_r)
    df_runs.to_csv(path_to_data + f'/df_aupr_{c}.csv')
